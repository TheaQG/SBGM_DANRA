...
def compute_daily_metrics_over_time(dict_data1, dict_data2):
    """
    Accepts dictionaries mapping date -> 2D data arrays for two datasets.
    Computes metrics for each day and returns a list of dicts.
    """
    timeseries = []
    for date in sorted(dict_data1.keys()):
        if date not in dict_data2:
            continue
        data1 = dict_data1[date]
        data2 = dict_data2[date]
...
def compare_over_time(dict_data1, dict_data2, model1, model2, variable, save_path='./figures', show=False):
    ...
    for metric in metrics:
        ...
        summary_stats[f"{metric}_mean"] = mean(values)
        summary_stats[f"{metric}_std"] = stdev(values) if len(values) > 1 else 0.0
