...
@contextmanager
def paper_rc(big: float = 1.35, base: int = 11):
    """
    Local matplotlib style context with bigger fonts/lines for multi-panel figures.
    big: overall scaling factor (1.25–1.6 typical)
    base: base font size in points
    """
    fs = lambda x: max(1, int(round(x)))

    rc = {
        # Global
        "font.size":               fs(base * big),
        "figure.titlesize":        fs((base + 5) * big),
        "axes.titlesize":          fs((base + 3) * big),
        "axes.labelsize":          fs((base + 2) * big),
        "xtick.labelsize":         fs((base + 0) * big),
        "ytick.labelsize":         fs((base + 0) * big),
        "legend.fontsize":         fs((base + 0) * big),
        "legend.title_fontsize":   fs((base + 0) * big),
        ...
        # Layout
        "figure.constrained_layout.use": True,   # helps when saving side-by-side
        "savefig.bbox":            "tight",
    ...
    *,
    ...
    def _add_colorbar_and_boxplot(fig, ax, im, img_2d, vlim):
        ...
        if vals.size:
            ...
            # Make boxplot line widths readable in multi-panel figures
            for spine in bax.spines.values():
                spine.set_linewidth(plt.rcParams.get("axes.linewidth", 1.0))            
    ...
    def _plot_one_figure(day_list):
        # Use local rcParams scaling so text stays readable in side-by-side LaTeX figures
        with paper_rc(big=1.45, base=11):
            n_rows = len(day_list)
            ...
            for r, d in enumerate(day_list):
                ...
                for c, (fld, model_label, cmap, lims) in enumerate(panels):
                    ...
                    if c == 0:
                        ax.set_ylabel(d.strftime("%Y-%m-%d"))
                    ...
                    if add_dk_outline:
                        try:
                            ...
                            if mask is not None:
                                ...
                                overlay_outline(ax, mask, color="darkgrey", linewidth=1.2)
                        except Exception:
                            pass
