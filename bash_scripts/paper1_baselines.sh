...
#SBATCH --output=logs/slurm_baselines_%x_%j.log
#SBATCH --error=logs/slurm_baselines_%x_%j.err
...
TYPE=${3:-bilinear}   # bilinear | qm | unet_sr
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -u sbgm.cli.main_app --config_path '$CONFIG_DIR/$CFG' --mode baseline 
"
