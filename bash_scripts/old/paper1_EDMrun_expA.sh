#!/bin/bash
#SBATCH --job-name=paper1_EDM_RG
#SBATCH --output=logs/slurm_paper1_EDM_RG_%x_%j.log
#SBATCH --error=logs/slurm_paper1_EDM_RG_%x_%j.err
...
srun singularity exec "$CONTAINER" bash -lc "
  ...
  python -m sbgm.cli.main_app --mode quicklook --config_path '$CONFIG_DIR/paper1_mainRG.yaml' --make_plots &&
  python -m sbgm.cli.main_app --mode generate  --config_path '$CONFIG_DIR/paper1_mainRG.yaml' --make_plots &&
  python -m sbgm.cli.main_app --mode evaluate  --config_path '$CONFIG_DIR/paper1_mainRG.yaml' --make_plots 
"

  # python -m sbgm.cli.main_app --mode full_pipeline --config_path $CONFIG_DIR/paper1_mainRG.yaml --make_plots