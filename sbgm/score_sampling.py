...
#   B = int(batch_size)
#   # Initial sample: Gaussian noise with sigma_max stddev
#   x = torch.randn(B, C_out, H, W, device=device) * float(sigmas[0]) # Initialize with first schedule sigma, not raw sigma_max
#   # Alternative (fixed) initialization: constant noise (for reproducibility)
#   # x = torch.randn(B, C_out, H, W, device=device) * float(sigma_max)
...
#   # Check if the model predicts residuals (x0) or noise; default: False (predict x0)
#   predict_residual = bool(getattr(score_model, 'predict_residual', False) or (getattr(score_model, 'cfg', {}).get('edm', {}).get('predict_residual', False)))

#   # Helper: add baseline if model predicts residuals
#   def add_baseline_if_residual(x0_pred, baseline):
#     return x0_pred + baseline if (predict_residual and baseline is not None) else x0_pred
...
#                           y=null_y,
#                           lr_ups=null_lr_ups)
#       x0_uc = add_baseline_if_residual(x0_uc, baseline=null_lr_ups)
...
#                         y=y,
#                         lr_ups=lr_ups)
#       x0_c = add_baseline_if_residual(x0_c, baseline=lr_ups)

#       denoised = x0_uc + s * (x0_c - x0_uc) 
#     else:
#       denoised = score_model(x_in,
#                             sigma_vec,
#                             cond_img=cond_img,
...
#                             y=y,
#                             lr_ups=lr_ups)
#       denoised = add_baseline_if_residual(denoised, baseline=lr_ups)
#     return denoised

#   # # Alternative version (OLD)
#   # def _denoise_with_cfg(x_in, sigma_vec):
#   #   if cfg_enabled and base_scale > 0.0:
