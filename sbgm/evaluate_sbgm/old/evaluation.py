...
class Evaluation:
    ...
    def __init__(self, cfg, generated_sample_type='repeated', n_samples=4):
        ...
        # Load conditional images (support dual-LR)
        self.cond_imgs = []       # will hold tensors of shape [B, ch_v, H, W]
        for v in self.lr_vars:
            base = os.path.join(self.generated_sample_path, f'cond_samples_{v}{load_str}')
            if os.path.exists(base):
                arr = np.load(base)['arr_0']   # [B,H,W]
                ten = torch.from_numpy(arr).unsqueeze(1)  # [B,1,H,W]
                self.cond_imgs.append(ten)
            else:
                ch0 = os.path.join(self.generated_sample_path, f'cond_samples_{v}_ch0{load_str}')
                ch1 = os.path.join(self.generated_sample_path, f'cond_samples_{v}_ch1{load_str}')
                if os.path.exists(ch0) and os.path.exists(ch1):
                    a0 = torch.from_numpy(np.load(ch0)['arr_0']).unsqueeze(1)  # [B,1,H,W]
                    a1 = torch.from_numpy(np.load(ch1)['arr_0']).unsqueeze(1)
                    self.cond_imgs.append(torch.cat([a0, a1], dim=1))         # [B,2,H,W]
                else:
                    raise FileNotFoundError(f"Missing conditional files for {v}: {base} or {ch0}+{ch1}")

        # Concatenate all conditionals into [B, C_total, H, W]
        self.cond_imgs = torch.cat(self.cond_imgs, dim=1)
    ...
    def _apply_land_mask(self, t: torch.Tensor) -> torch.Tensor:
        # t : [B,H,W] or [B,1,H,W]; self.lsm_imgs is numpy [B,H,W] or None
        if getattr(self.cfg.evaluation, 'eval_land_only', False) and (self.lsm_imgs is not None):
            m = torch.from_numpy(self.lsm_imgs).to(t.device).float()
            if t.ndim == 4 and t.shape[1] == 1:
                t = t[:,0]
            t = torch.where(m > 0.5, t, torch.tensor(float('nan'), device=t.device, dtype=t.dtype))
        return t
    ...
    def full_pixel_statistics(self,
        ...
        gen = self._apply_land_mask(self.gen_imgs.clone())
        evl = self._apply_land_mask(self.eval_imgs.clone())
        gen_imgs_flat = torch.nan_to_num(gen).flatten()
        eval_imgs_flat = torch.nan_to_num(evl).flatten()
        # OR: build boolean mask and index only valid land pixels to truly ignore NaNs:
        # mask = torch.isfinite(gen) & torch.isfinite(evl)
        # gen_imgs_flat = gen[mask]; eval_imgs_flat = evl[mask]
    ...
    def spatial_statistics(self,
        ...
        rmse_per_pixel = torch.sqrt(torch.square(gen - evl).nanmean(dim=0))

        # Calculate mae per pixel
        mae_per_pixel = torch.abs(gen - evl).nanmean(dim=0)

        # Calculate bias per pixel
        bias_per_pixel = gen.nanmean(dim=0) - evl.nanmean(dim=0)
